...
class PolicyValidator:
    ...
    def identify_duplicate_policies(self, fw_policy_df: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
        ...
        duplicate_mask = fw_policy_df.duplicated(
            subset=["src_ip", "dst_ip", "protocol", "port", "action"], keep="first"
        )
    ...
    def validate_protocol_port_combinations(self, fw_policy_df: pd.DataFrame) -> List[str]:
        ...
        icmp_with_ports = icmp_policies[
            icmp_policies["port"].notna() & (icmp_policies["port"] != "")
        ]
    ...
    def perform_comprehensive_validation(
        ...
        total_issues = (
            stateless_issues_count + len(unused_tags) + duplicate_count + len(validation_warnings)
        )
