...
class SmartGroupBuilder:
    ...
    def build_cidr_smartgroups(self, fw_policy_df: pd.DataFrame, fw_tag_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        for cidr in actual_cidrs:
            cidr_sgs.append({
                'selector': {'match_expressions': {'cidr': cidr}},
                'name': f"cidr_{cidr}"
            })
    ...
    def build_vpc_smartgroups(self, gateways_df: pd.DataFrame) -> pd.DataFrame:
        ...
        vpcs['selector'] = vpcs.apply(lambda row: {
            'match_expressions': {
    ...
    def build_smartgroup_df(self, fw_policy_df: pd.DataFrame, fw_tag_df: pd.DataFrame, gateways_df: pd.DataFrame) -> pd.DataFrame:
        """
...
class SmartGroupManager:
    ...
    def create_all_smartgroups(self, fw_policy_df: pd.DataFrame, fw_tag_df: pd.DataFrame, 
                              gateways_df: pd.DataFrame, hostname_rules_df: Optional[pd.DataFrame] = None) -> Dict[str, pd.DataFrame]:
        """
        ...
        standard_smartgroups = self.builder.build_smartgroup_df(fw_policy_df, fw_tag_df, gateways_df)
        ...
        if hostname_rules_df is not None and not hostname_rules_df.empty:
            ...
            complete_smartgroups = pd.concat([standard_smartgroups, hostname_sg_for_export], ignore_index=True)
        ...
        # Log summary
        self.logger.info(f"SmartGroup creation summary:")
