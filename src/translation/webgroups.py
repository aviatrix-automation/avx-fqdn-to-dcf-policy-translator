...
from ..domain.constants import FQDN_MODE_MAPPINGS, DCF_CONSTRAINTS
from ..domain.validators import filter_domains_for_dcf_compatibility
...
class WebGroupBuilder:
    ...
    def filter_and_create_selector(self, row: pd.Series) -> Dict[str, Any]:
        ...
        valid_domains, invalid_domains = filter_domains_for_dcf_compatibility(
        ...
        if invalid_domains:
            ...
            self.all_invalid_domains.extend([
                {'webgroup': webgroup_name, 'domain': domain} 
    ...
    def build_webgroup_dataframe(self, fqdn_tag_rule_df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouped_df = fqdn_tag_rule_df.groupby([
            'fqdn_tag_name', 'protocol', 'port', 'fqdn_mode'
...
class WebGroupManager:
    ...
    def validate_webgroup_constraints(self, webgroups_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(long_names) > 0:
            logging.warning(f"Found {len(long_names)} WebGroups with names exceeding {max_name_length} characters")
            for _, row in long_names.iterrows():
                logging.warning(f"WebGroup name too long: '{row['name'][:50]}...' (length: {len(row['name'])})")
        ...
        if len(valid_webgroups) < len(webgroups_df):
            logging.warning(f"Removed {len(webgroups_df) - len(valid_webgroups)} WebGroups due to constraint violations")
