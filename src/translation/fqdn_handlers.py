...
from domain.constants import DOMAIN_PATTERNS
...
class FQDNValidator:
    ...
    @staticmethod
    def filter_domains_for_dcf_compatibility(fqdn_list: List[str], 
                                            webgroup_name: str = None) -> Tuple[List[str], List[str]]:
        """
...
class FQDNRuleProcessor:
    ...
    def __init__(self, default_web_port_ranges: List[str]):
        ...
        Args:
            default_web_port_ranges: List of port ranges considered web traffic (e.g., ['80', '443'])
        """
        self.default_web_port_ranges = set(default_web_port_ranges)
    
    def eval_unsupported_webgroups(self, fqdn_tag_rule_df: pd.DataFrame, 
                                 fqdn_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
...
class WebGroupBuilder:
    ...
    def build_webgroup_df(self, fqdn_tag_rule_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def filter_and_create_selector(row):
            ...
            valid_domains, invalid_domains = FQDNValidator.filter_domains_for_dcf_compatibility(row['fqdn'], webgroup_name)
...
class HostnameSmartGroupBuilder:
    ...
    def build_hostname_smartgroups(self, hostname_rules_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(hostname_rules_df) == 0:
            return pd.DataFrame(columns=['name', 'selector', 'protocol', 'port', 'fqdn_mode', 'fqdn_list'])
...
class FQDNPolicyBuilder:
    ...
    def __init__(self, translate_port_to_port_range_func, 
                 pretty_parse_vpc_name_func, deduplicate_policy_names_func):
        """
    ...
    def build_hostname_policies(self, gateways_df: pd.DataFrame, fqdn_df: pd.DataFrame, 
        ...
        egress_vpcs['src_smart_groups'] = self.pretty_parse_vpc_name(egress_vpcs, "src_smart_groups")
...
class FQDNHandler:
    ...
    def process_fqdn_rules(self, fqdn_tag_rule_df: pd.DataFrame, 
                          fqdn_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
    ...
    def filter_domains(self, domains: List[str], context: str = None) -> Tuple[List[str], List[str]]:
        """
