...
from config import INVALID_CHARS_REPLACEMENT, TranslationConfig
    ...
    def remove_policy_duplicates(self, fw_policy_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if duplicates.any():
            ...
            if self.config.enable_debug:
                ...
                self.logger.info(
                    f"Exported {len(removed_duplicates)} duplicate policies to {debug_file}"
                )
...
class FirewallTagProcessor:
    ...
    def eval_single_cidr_tag_match(
        self, fw_policy_df: pd.DataFrame, fw_tag_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
    ...
    def eval_stateless_alerts(self, fw_policy_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not stateless_alerts.empty:
            ...
            if self.config.enable_debug:
                ...
                self.logger.warning(
                    f"Exported {len(stateless_alerts)} stateless policy issues to {issues_file}"
                )
...
class DataProcessor:
    ...
    def process_firewall_policies(
        ...
        processed_policies = self.fw_tag_processor.eval_single_cidr_tag_match(
            fw_policy_df, cleaned_tags
        )
        ...
        self.logger.info(
            f"Policy processing complete: {len(final_policies)} clean policies, "
            f"{len(cleaned_tags)} tags"
        )
