...
class TerraformExporter:
    ...
    def export_dataframe_to_tf(self, df: pd.DataFrame, resource_name: str, name_column: str) -> Path:
        """
        ...
        if df.empty:
            ...
            tf_resource_dict = {'resource': {resource_name: {}}}
        else:
        ...
        output_file = self.config.output_dir / f"{resource_name}.tf.json"
    ...
    def export_policies_to_tf(self, policies_df: pd.DataFrame) -> Path:
        ...
        if policies_df.empty:
            ...
            policy_dict = {'resource': {'aviatrix_distributed_firewalling_policy_list': {
        ...
        output_file = self.config.get_output_file_path('dcf_policies')
...
provider "aviatrix" {
  skip_version_validation = true
        ...
        output_file = self.config.get_output_file_path('main_tf')
...
class CSVExporter:
    ...
    def export_to_csv(self, df: pd.DataFrame, filename: str, directory: str = "output") -> Path:
        ...
        if directory == "debug":
            output_file = self.config.debug_dir / filename
        else:
...
class ReportExporter:
    ...
    def export_summary_report(self, summary: Dict[str, Any]) -> Path:
        ...
        """
        output_file = self.config.output_dir / 'translation_summary.json'
    ...
    def export_all_outputs(self, data: Dict[str, Any]) -> Dict[str, Path]:
        ...
        try:
            ...
            if 'full_policy_list' in data:
                exported_files['policies_tf'] = self.tf_exporter.export_policies_to_tf(data['full_policy_list'])
