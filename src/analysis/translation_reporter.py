...
class TranslationReporter:
    ...
    def generate_fqdn_report(self, fqdn_analysis: FQDNAnalysisResult) -> Dict[str, Any]:
        ...
        report = {
            ...
            'dcf_translation_breakdown': {
                ...
                'webgroup_percentage': fqdn_analysis.webgroup_rules / max(fqdn_analysis.enabled_rules, 1) * 100,
                'hostname_percentage': fqdn_analysis.hostname_rules / max(fqdn_analysis.enabled_rules, 1) * 100
            },
            'domain_compatibility': {
                ...
                'compatibility_rate': fqdn_analysis.dcf_compatible_domains / max(fqdn_analysis.unique_domains, 1)
            },
            'breakdown_analysis': {
    ...
    def generate_validation_report(self, validation_result: ValidationResult) -> Dict[str, Any]:
        ...
        report = {
            'validation_summary': {
                ...
                'validation_success_rate': 1 - (validation_result.issues_found / max(validation_result.total_policies, 1))
            },
            'issue_breakdown': {
    ...
    def export_summary_to_text(self, report: Dict[str, Any], filename: str = None) -> Path:
        """
        ...
        try:
            with open(output_file, 'w') as f:
                ...
                if 'report_metadata' in report:
                    f.write(f"Generated: {report['report_metadata'].get('generated_at', 'Unknown')}\\n")
                    f.write(f"Version: {report['report_metadata'].get('report_version', 'Unknown')}\\n\\n")
                ...
                if 'fqdn_analysis' in report:
                    ...
                    f.write(f"  Hostname SmartGroup rules: {dcf_breakdown.get('hostname_smartgroup_rules', 0)}\\n")
                ...
                if 'validation_analysis' in report:
                    ...
                    f.write(f"Total policies analyzed: {validation_summary.get('total_policies_analyzed', 0)}\\n")
                    f.write(f"Issues found: {validation_summary.get('total_issues_found', 0)}\\n")
                    f.write(f"Success rate: {validation_summary.get('validation_success_rate', 0):.2%}\\n")
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if 'validation_analysis' in report:
            ...
            if issue_breakdown.get('unused_firewall_tags', 0) > 0:
                recommendations.append(
                    "Remove unused firewall tags to simplify configuration"
                )
        ...
        if not recommendations:
            recommendations.append("Translation completed successfully with no major issues identified")
