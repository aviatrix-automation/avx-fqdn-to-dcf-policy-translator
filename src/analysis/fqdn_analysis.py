...
class FQDNCategorizer:
    ...
    def __init__(self, default_web_ports: Set[str] = None):
        """
    ...
    def categorize_by_protocol_port(self, fqdn_rules_df: pd.DataFrame, fqdn_df: pd.DataFrame = None) -> Dict[str, pd.DataFrame]:
        """
        ...
        if fqdn_df is not None and not fqdn_df.empty:
            ...
            enabled_tags = set(fqdn_df[fqdn_df['fqdn_enabled'] == True]['fqdn_tag'].tolist())
    ...
    def categorize_by_gateway(self, fqdn_rules_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        if 'gw_filter_tag_list' in fqdn_rules_df.columns:
            for idx, row in fqdn_rules_df.iterrows():
...
class DomainCompatibilityAnalyzer:
    ...
    def analyze_webgroup_domains(self, webgroups_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for idx, row in webgroups_df.iterrows():
...
class FQDNAnalyzer:
    ...
    def analyze_fqdn_rules(self, fqdn_tag_rule_df: pd.DataFrame, 
                          fqdn_df: pd.DataFrame) -> FQDNAnalysisResult:
        """
        ...
        enabled_rules = len(fqdn_df[fqdn_df['fqdn_enabled'] == True]) if not fqdn_df.empty else 0
        disabled_rules = len(fqdn_df[fqdn_df['fqdn_enabled'] == False]) if not fqdn_df.empty else 0
        ...
        if not fqdn_df.empty and 'gw_filter_tag_list' in fqdn_df.columns:
            for idx, row in fqdn_df.iterrows():
                gw_filter = row.get('gw_filter_tag_list', {})
                if isinstance(gw_filter, dict) and 'gw_name' in gw_filter:
                    gw_name = gw_filter['gw_name']
    ...
    def generate_analysis_report(self, analysis_result: FQDNAnalysisResult) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'enablement_rate': analysis_result.enabled_rules / analysis_result.total_rules if analysis_result.total_rules > 0 else 0
            },
            'dcf_translation': {
                ...
                'webgroup_rate': analysis_result.webgroup_rules / analysis_result.enabled_rules if analysis_result.enabled_rules > 0 else 0,
                'hostname_rate': analysis_result.hostname_rules / analysis_result.enabled_rules if analysis_result.enabled_rules > 0 else 0
            },
            'domain_analysis': {
                ...
                'compatibility_rate': analysis_result.dcf_compatible_domains / analysis_result.unique_domains if analysis_result.unique_domains > 0 else 0
            },
            'breakdowns': {
    ...
    def log_analysis_summary(self, analysis_result: FQDNAnalysisResult) -> None:
        ...
        Args:
            analysis_result: Result of FQDN analysis
        """
        self.logger.info(f"FQDN Analysis Summary:")
        ...
        self.logger.info(f"  DCF 8.0 incompatible domains: {analysis_result.dcf_incompatible_domains}")
        
        if analysis_result.dcf_incompatible_domains > 0:
            self.logger.warning(f"Found {analysis_result.dcf_incompatible_domains} domains incompatible with DCF 8.0 SNI requirements")
